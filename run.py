...
class QuotesScraper:
	...
	def page_scrape(self, url):
		...
		data_to_scrap = soup.find_all("script")[1].text.split("for (var i in data)")[0].split("var data =")[-1].strip().rstrip(';')
	...
	def quote_scrape(self):
		...
		while True:
			...
			next_page = BeautifulSoup(requests.get(url).text, "html.parser").find("li", {"class": "next"})
			if not next_page:
				break
			else:
